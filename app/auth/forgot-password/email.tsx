...
export default function ForgotPasswordByEmailScreen() {
  ...
  const { setEmail } = useIdentityActions()
  ...
  }, [email, expireTime, setEmail, setExpireTime, setStep, initiateForgotPassword, tToast])
  ...
  const handleResendOTP = useCallback(() => {
    resendOTPForgotPassword(
      { email: email, verificationMethod: VerificationMethod.EMAIL },
      {
        onSuccess: (response) => {
          showToast(tToast('toast.sendVerifyEmailSuccess'))
          setExpireTime(response?.result?.expiresAt || '')
