...
export default function OrderTypeSelect() {
  ...
  return (
    <Select value={selectedType?.value} onValueChange={handleChange}>
      <Select.Trigger className="w-full bg-white dark:bg-gray-900">
        <Select.Value placeholder={t('menu.selectOrderType')} />
      </Select.Trigger>
      <Select.Content className={getTheme() === 'dark' ? 'bg-gray-900' : 'bg-white'}>
        {orderTypes.map((type) => (
